...
## How it works
- `LidSensor.swift`: reads the HID hinge sensor (Apple 0x05AC/0x8104); no permissions required. A poll thread does the blocking HID call (≈0.6 ms), so animation frames never wait on the hardware
- `Engine.swift`: puts the overlay up in its "closed" state (fully frosted) before the Mac sleeps, so the first frame after waking already shows the effect; after that, an α-β filter plus a critically damped spring follow the lid angle. While animating it runs on a CVDisplayLink bound to the built-in display (120 Hz even when a 60 Hz external monitor is the main display) and computes each frame for its presentation time
